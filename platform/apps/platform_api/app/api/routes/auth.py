...
async def register(
    ...
    rate_limit_state = await check_registration_rate_limit(request, settings=settings)
    ...
    if existing_user is not None:
        can_restart_pending_registration = (
            verification_required
            and existing_user.status == "pending_verification"
            and existing_user.email_verified_at is None
        )

        if not can_restart_pending_registration:
            raise HTTPException(
        ...
        player_profile = await db_session.scalar(
        ...
        if player_profile is None:
            ...
            player_profile.display_name = user.display_name
            player_profile.contact_email = user.email
    ...
    roles = await db_session.scalars(select(Role).where(Role.slug.in_(role_slugs)))
    resolved_roles = list(roles)
    if {role.slug for role in resolved_roles} != set(role_slugs):
        raise HTTPException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
    ...
    await write_audit_log(
        ...
        payload={
            ...
            "pending_account_restarted": recovered_pending_account,
        },
    )
