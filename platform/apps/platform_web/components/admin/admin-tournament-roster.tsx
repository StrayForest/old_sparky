...
export function AdminTournamentRoster({ slug, formatDate }: { slug: string; formatDate: (value: string) => string }) {
  ...
  useEffect(() => {
    ...
    setAddTeamKey("");
    setAddSlot("1");
    setDestinationTeamKey("");
    setDestinationSlot("1");
    setReplacementUserId("");
    setOverride(false);

    void getAdminTournamentRoster(slug, controller.signal).then((value) => {
      if (!ownsLoad(generation, slug, controller)) return;
      if (!value || value.tournament_slug !== slug || typeof value.state_version !== "number" || !Array.isArray(value.teams) || !Array.isArray(value.unassigned_participants)) {
        setLoadError(t("admin.new.rosterLoadFailed"));
        return;
      }
      setRoster(value);
    }).catch((error: unknown) => {
      if (ownsLoad(generation, slug, controller) && !controller.signal.aborted) setLoadError(platformApiMessage(error, t("admin.new.rosterLoadFailed")));
    }).finally(() => {
      if (ownsLoad(generation, slug, controller)) {
  ...
  function ownsLoad(generation: number, requestedSlug: string, controller: AbortController): boolean {
    return requestGenerationRef.current === generation
      && currentSlugRef.current === requestedSlug
      && loadControllerRef.current === controller;
  }

  function ownsMutation(generation: number, requestedSlug: string, controller: AbortController): boolean {
    return requestGenerationRef.current === generation
      && currentSlugRef.current === requestedSlug
  ...
  async function runMutation(operation: AdminRosterMutationOperation, fields: Record<string, unknown>) {
    ...
    try {
      const updated = await mutateAdminTournamentRoster(requestSlug, operation, { expected_state_version: requestRoster.state_version, reason: reason.trim(), override: requestRoster.capabilities.requires_override && override, ...fields }, controller.signal);
      if (!ownsMutation(generation, requestSlug, controller) || controller.signal.aborted || updated.tournament_slug !== requestSlug || updated.tournament_id !== requestRoster.tournament_id) return;
      setRoster(updated); setReason(""); setSelectedMemberId(""); setSuccess(t("admin.new.rosterSaved"));
    } catch (error: unknown) {
      if (ownsMutation(generation, requestSlug, controller) && !controller.signal.aborted) setActionError(error instanceof PlatformApiError && error.status === 409 ? t("admin.new.rosterStale") : platformApiMessage(error, t("admin.new.rosterLoadFailed")));
    } finally {
      if (ownsMutation(generation, requestSlug, controller)) {
        mutationControllerRef.current = null;
        setBusy(null);
      }
    }
  ...
  if (isLoading) return <div className="ops-detail-content ops-roster-state" aria-busy="true" role="status"><RefreshCcw className="ops-spin" size={17} />{t("common.loading")}</div>;
  if (!roster) return <div className="ops-detail-content ops-roster-state" role="alert"><UsersRound size={19} /><span>{loadError || t("admin.new.rosterNoMaterialized")}</span></div>;
  const blocked = Boolean(roster.capabilities.blocked_reason);
  return <div className="ops-detail-content ops-roster" data-testid="admin-roster-panel" aria-busy={Boolean(busy)}>
    ...
    <div className="ops-roster-teams"><div className="ops-subsection-label">{t("admin.new.currentTeams")}</div>{roster.teams.length ? roster.teams.map((team) => <div className="ops-team-row" key={team.id}><div className="ops-team-header"><div><strong>{team.name}</strong><small>{team.team_key}</small></div><span>{team.starter_average_strength.toFixed(1)}</span></div><div className="ops-team-members">{team.members.map((member) => <button className={member.id === selectedMemberId ? "ops-member-row is-selected" : "ops-member-row"} disabled={Boolean(busy)} key={member.id} type="button" onClick={() => { setSelectedMemberId(member.id); setDestinationTeamKey(team.team_key); setDestinationSlot(String(member.slot_number === 6 ? 1 : member.slot_number)); }}><span className="ops-member-slot">{slotLabel(member)}</span><span><strong>{member.display_name}</strong><small>{member.rank ?? "—"} · {member.assigned_role ?? "—"}</small></span>{member.roster_role === "captain" ? <Crown size={14} /> : null}</button>)}</div></div>) : <div className="ops-empty-inline">{t("admin.new.rosterNoTeams")}</div>}</div>
    <div className="ops-roster-form"><div className="ops-subsection-label">{t("admin.new.addPlayer")}</div><div className="ops-form-grid-roster"><label className="ops-field"><span>{t("admin.new.player")}</span><select disabled={Boolean(busy)} value={addUserId} onChange={(event) => setAddUserId(event.target.value)}><option value="">{t("admin.new.selectPlayer")}</option>{roster.unassigned_participants.map((player) => <option key={player.user_id} value={player.user_id}>{player.display_name} · {player.rank ?? "—"}</option>)}</select></label><label className="ops-field"><span>{t("admin.new.team")}</span><select disabled={Boolean(busy)} value={addTeamKey} onChange={(event) => setAddTeamKey(event.target.value)}>{roster.teams.map((team) => <option key={team.team_key} value={team.team_key}>{team.name}</option>)}</select></label><label className="ops-field"><span>{t("admin.new.slot")}</span><select disabled={Boolean(busy)} value={addSlot} onChange={(event) => setAddSlot(event.target.value)}>{[1, 2, 3, 4, 5, 6].map((slot) => <option key={slot} value={slot}>{slot === 6 ? "S" : slot}</option>)}</select></label><button className="ops-button ops-button-secondary" type="button" disabled={!addUserId || !canRun("add-player")} onClick={() => void runMutation("add-player", { user_id: addUserId, team_key: addTeamKey, slot_number: Number(addSlot) })}>{busy === "add-player" ? <RefreshCcw className="ops-spin" size={15} /> : <ArrowRight size={15} />}{t("admin.new.add")}</button></div></div>
    {selected ? <div className="ops-roster-form"><div className="ops-subsection-label">{t("admin.new.selectedPlayer", { name: selected.member.display_name })}</div><div className="ops-form-grid-roster"><label className="ops-field"><span>{t("admin.new.team")}</span><select disabled={Boolean(busy)} value={destinationTeamKey} onChange={(event) => setDestinationTeamKey(event.target.value)}>{roster.teams.map((team) => <option key={team.team_key} value={team.team_key}>{team.name}</option>)}</select></label><label className="ops-field"><span>{t("admin.new.slot")}</span><select disabled={Boolean(busy)} value={destinationSlot} onChange={(event) => setDestinationSlot(event.target.value)}>{[1, 2, 3, 4, 5, 6].map((slot) => <option key={slot} value={slot}>{slot === 6 ? "S" : slot}</option>)}</select></label><label className="ops-field"><span>{t("admin.new.replacement")}</span><select disabled={Boolean(busy)} value={replacementUserId} onChange={(event) => setReplacementUserId(event.target.value)}><option value="">{t("admin.new.selectPlayer")}</option>{roster.unassigned_participants.map((player) => <option key={player.user_id} value={player.user_id}>{player.display_name}</option>)}</select></label></div><div className="ops-action-row"><button className="ops-button ops-button-secondary" type="button" disabled={!canRun("move-player") || selected.member.roster_role === "captain"} onClick={() => void runMutation("move-player", { team_key: selected.team.team_key, user_id: selected.member.user_id, destination_team_key: destinationTeamKey, destination_slot: Number(destinationSlot) })}><ArrowRight size={15} />{t("admin.new.move")}</button><button className="ops-button ops-button-secondary" type="button" disabled={!replacementUserId || !canRun("replace-player")} onClick={() => void runMutation("replace-player", { team_key: selected.team.team_key, slot_number: selected.member.slot_number, replacement_user_id: replacementUserId })}>{t("admin.new.replace")}</button><button className="ops-button ops-button-secondary" type="button" disabled={!canRun("change-captain") || selected.member.roster_role === "captain"} onClick={() => void runMutation("change-captain", { team_key: selected.team.team_key, user_id: selected.member.user_id })}><Crown size={15} />{t("admin.new.captain")}</button><button className="ops-button ops-button-danger" type="button" disabled={!canRun("remove-player") || selected.member.roster_role === "captain"} onClick={() => void runMutation("remove-player", { team_key: selected.team.team_key, user_id: selected.member.user_id })}>{t("admin.new.remove")}</button></div></div> : <div className="ops-muted">{t("admin.new.selectPlayerHint")}</div>}
    <label className="ops-field"><span>{t("admin.new.reason")}</span><textarea disabled={Boolean(busy)} maxLength={1000} value={reason} placeholder={t("admin.new.rosterReasonPlaceholder")} onChange={(event) => setReason(event.target.value)} /></label>{roster.capabilities.requires_override && roster.capabilities.can_override ? <label className="ops-checkbox"><input disabled={Boolean(busy)} type="checkbox" checked={override} onChange={(event) => setOverride(event.target.checked)} />{t("admin.new.overrideRoster")}</label> : null}{actionError ? <div className="ops-feedback ops-feedback-error" role="alert">{actionError}</div> : null}{success ? <div className="ops-feedback ops-feedback-success" aria-live="polite">{success}</div> : null}
  </div>;
}
