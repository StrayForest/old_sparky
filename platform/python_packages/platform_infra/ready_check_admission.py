...
READY_CHECK_ADMISSION_TTL_SECONDS = 900
...
def _parse(
    ...
    *,
    ...
    if (
        ...
        or expires_at - issued_at > READY_CHECK_ADMISSION_TTL_SECONDS
    ):
...
def issue_ready_check_state_proof(
    *,
    ...
    expires_at = min(ends_at + READY_CHECK_ADMISSION_CLOCK_SKEW_SECONDS, issued_at + READY_CHECK_ADMISSION_TTL_SECONDS)
    ...
    return _issue(
        {
            ...
            "iat": issued_at,
            "exp": issued_at + READY_CHECK_ADMISSION_TTL_SECONDS,
        }
    )
...
def verify_ready_check_stream_proof(
    ...
    *,
    ...
    issued_at = int(payload["iat"])
    expires_at = int(payload["exp"])
    if admission_open_at < issued_at or admission_open_at > expires_at:
