...
_shared_clients: dict[bool, tuple[asyncio.AbstractEventLoop, Redis]] = {}
...
def redis_client(
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "Shared Redis clients require a running event loop."
        ) from exc
    ...
    if entry is not None:
        raise RuntimeError(
            "Shared Redis client belongs to another event loop; "
            "dispose it on its owning loop before creating a replacement."
        )
...
async def dispose_redis_clients() -> None:
    ...
    current_loop = asyncio.get_running_loop()
    if any(loop is not current_loop for loop, _client in _shared_clients.values()):
        raise RuntimeError(
            "Shared Redis clients must be disposed on their owning event loop."
        )
