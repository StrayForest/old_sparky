...
- Candidate `c7da9230` passed public 1k mixed (300/300 SSE 200, 300/300 events, 1,783 polling requests, zero errors; SSE connect p95 5.8s). Candidate `a3908a45` reduced early pool waits and coalesced public checks, but its public mixed 10k run still hit 500s after 1,691 SSE; VPS snapshots showed ordinary and stream work sharing API pool 12. The next candidate uses a bounded SSE-only pool of two connections per API worker, isolating admission/revalidation from polling while retaining fail-closed private checks.
