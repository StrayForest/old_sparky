...
- Candidate `b39dccee` passed the corrected public 1k mixed stage (1,000/1,000 SSE 200 and events, 1,783 polling requests, zero errors; SSE connect p95 16.7s). Its 10k mixed fixture with 1k SSE still exhausted the API pool with five-second checkout 500s, while 5k SSE attempts were stopped at the deliberate 3,000-stream app cap and produced fast 429s, not Cloudflare 1200. Release `2ca7a8df` increased only the bounded API pool from 12 to 16 per worker and the total budget from 32 to 40, counting the separate SSE pool explicitly; the live 10k mixed run still exhausted pool16. VPS request telemetry identified optional read authentication's isolated `last_seen_at` write as a second checkout during the read burst. The next public A/B removes that non-authoritative write from optional read auth while retaining mutation touches and SSE revalidation; no Cloudflare bypass or admission increase is part of the change.
