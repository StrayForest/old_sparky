...
- Candidate `c7da9230` passed public 1k mixed (300/300 SSE 200, 300/300 events, 1,783 polling requests, zero errors; SSE connect p95 5.8s). Its public mixed 10k run still hit API `QueuePool` 500s while SSE stayed 200: one worker exhausted pool 12 with six stream-admission DB sessions plus polling. The next candidate lowers admission/revalidation to two per worker and coalesces public checks by tournament/slug; private viewer-specific checks remain fail-closed.
