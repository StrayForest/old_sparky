...
The selectable runtime A/B profiles are `uvicorn-classic`, `uvicorn-optimized`, `pool-pre-ping-off`, `authenticated-read-admission-32`, `web-ssr-diagnostics`, `web-ssr-native-transport`, `web-ssr-workers-2` and `api-pool-{12,16,20,24}`. Apply one at a time and restore the baseline; SSR diagnostics sample 1% of Next.js stages plus event-loop/Nginx timing and do not change SSR/CSP. The native transport profile changes only the trusted loopback auth GET implementation with one web worker; the workers-2 profile combines that transport with two web workers and remains an explicitly measured candidate. The diagnostic profile pins the API to the public `INFO`/performance-logging selectors so fast auth rows remain visible, then restarts `deadlock-api` because its sampled auth/API log gate is read at API startup. Profiles that change startup-read SSR flags restart `deadlock-web` after applying the shared environment so the selected state is not left stale in the existing Next.js process. Readiness is checked for both services, and the restore path clears the diagnostic gate and logging selectors. The `pool-pre-ping-off` recovery test is mandatory.
