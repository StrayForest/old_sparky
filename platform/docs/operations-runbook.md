...
The selectable runtime A/B profiles are `uvicorn-classic`, `uvicorn-optimized`, `pool-pre-ping-off`, `authenticated-read-admission-32`, `web-ssr-diagnostics` and `api-pool-{12,16,20,24}`. Apply one at a time and restore the baseline; SSR diagnostics sample 1% of Next.js stages plus event-loop/Nginx timing and do not change SSR/CSP. Profiles that change startup-read SSR flags restart `deadlock-web` after applying the shared environment so the selected state is not left stale in the existing Next.js process. `web-ssr-diagnostics` also restarts `deadlock-api` because its sampled auth/API log gate is read at API startup, and readiness is checked for both services. The `pool-pre-ping-off` recovery test is mandatory.
