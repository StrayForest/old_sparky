...
1. **AS-04 inactive-participant authorization** — withdrawn, rejected, disqualified or otherwise inactive participant rows must not retain private tournament workspace access.
2. **AS-03 invite/capacity serialization** — make invite use and participant-capacity check-and-write operations atomic under concurrency.
3. **AS-05 public/private data boundary** — prevent account contact and moderation/internal fields from crossing public API contracts.
4. **AS-06 SSE connection pressure** — add bounded per-source/global long-lived connection controls and regression coverage.
